...
package com.shpark.app.castcompanionlibrary.notification;
...
import com.shpark.app.castcompanionlibrary.cast.VideoCastManager;
import com.shpark.app.castcompanionlibrary.cast.callbacks.VideoCastConsumerImpl;
...
public class VideoCastNotificationService extends Service {
    ...
    public static final String ACTION_TOGGLE_PLAYBACK =
            "com.shpark.app.castcompanionlibrary.action.toggleplayback";
    public static final String ACTION_STOP =
            "com.shpark.app.castcompanionlibrary.action.stop";
    public static final String ACTION_VISIBILITY =
            "com.shpark.app.castcompanionlibrary.action.notificationvisibility";
    ...
    public void onCreate() {
        ...
        mConsumer = new VideoCastConsumerImpl() {
            ...
            public void onApplicationDisconnected(int errorCode) {
                LogUtils.LOGD(TAG, "onApplicationDisconnected() was reached");
                stopSelf();
            }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (null != intent) {
            ...
            if (ACTION_TOGGLE_PLAYBACK.equals(action) && mIsIcsOrAbove) {
                ...
                LogUtils.LOGD(TAG, "onStartCommand(): Action: ACTION_VISIBILITY " + mVisible);
    ...
    private void onRemoteMediaPlayerStatusUpdated(int mediaStatus) {
        mStatus = mediaStatus;
        LogUtils.LOGD(TAG, "onRemoteMediaPlayerMetadataUpdated() reached with status: " + mStatus);
        try {
            switch (mediaStatus) {
    ...
    private RemoteViews build(MediaInfo info, Bitmap bitmap, boolean isPlaying,
        ...
        RemoteViews rv = new RemoteViews(getPackageName(), com.shpark.app.castcompanionlibrary.R.layout.custom_notification);
        if (mIsIcsOrAbove) {
            addPendingIntents(rv, isPlaying, info);
        }
        if (null != bitmap) {
            rv.setImageViewBitmap(com.shpark.app.castcompanionlibrary.R.id.iconView, bitmap);
        }
        rv.setTextViewText(com.shpark.app.castcompanionlibrary.R.id.titleView, mm.getString(MediaMetadata.KEY_TITLE));
        String castingTo = getResources().getString(com.shpark.app.castcompanionlibrary.R.string.casting_to_device,
                mCastManager.getDeviceName());
        rv.setTextViewText(com.shpark.app.castcompanionlibrary.R.id.subTitleView, castingTo);
        mNotification = new NotificationCompat.Builder(this)
                .setSmallIcon(com.shpark.app.castcompanionlibrary.R.drawable.ic_stat_action_notification)
    ...
    private void addPendingIntents(RemoteViews rv, boolean isPlaying, MediaInfo info) {
        ...
        rv.setOnClickPendingIntent(com.shpark.app.castcompanionlibrary.R.id.playPauseView, playbackPendingIntent);
        rv.setOnClickPendingIntent(com.shpark.app.castcompanionlibrary.R.id.removeView, stopPendingIntent);

        if (isPlaying) {
            if (info.getStreamType() == MediaInfo.STREAM_TYPE_LIVE) {
                rv.setImageViewResource(com.shpark.app.castcompanionlibrary.R.id.playPauseView, com.shpark.app.castcompanionlibrary.R.drawable.ic_av_stop_sm_dark);
            } else {
                rv.setImageViewResource(com.shpark.app.castcompanionlibrary.R.id.playPauseView, com.shpark.app.castcompanionlibrary.R.drawable.ic_av_pause_sm_dark);
            }

        } else {
            rv.setImageViewResource(com.shpark.app.castcompanionlibrary.R.id.playPauseView, com.shpark.app.castcompanionlibrary.R.drawable.ic_av_play_sm_dark);
        }
    }
    ...
    private class DecodeVideoArtBitmapTask extends AsyncTask<MediaInfo, Void, Void> {
        ...
        protected Void doInBackground(final MediaInfo... info) {
            ...
            try {
                ...
                LogUtils.LOGE(TAG, "setIcon(): Failed to load the image with url: " +
        ...
        protected void onPostExecute(Void v) {
            try {
                ...
                LogUtils.LOGE(TAG, "Failed to set notification for " + mInfo.toString(), e);
            } catch (TransientNetworkDisconnectionException e) {
                LogUtils.LOGE(TAG, "Failed to set notification for " + mInfo.toString(), e);
            } catch (NoConnectionException e) {
                LogUtils.LOGE(TAG, "Failed to set notification for " + mInfo.toString(), e);
